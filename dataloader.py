...
class HandPoseDataset(Dataset):
    def __init__(
        ...
        self.min_depth_cam = 50.0
        self.max_depth_cam = 1500.0
        ...
        if drop_joint_num > 0:
            print(
                f"{drop_joint_num} out of {self.num_joints} joints will be dropped for each frame"
            )
            joint_list = [i for i in range(self.num_joints)]

            np.random.seed(random_seed + 10)
            self.dropped_joints = [
                np.random.choice(joint_list, drop_joint_num, replace=False)
    ...
    def __getitem__(self, index):
        ...
        gt2Dcrop = torch.from_numpy(
            (data["gt2Dcrop"] - np.array([0, 0, com[2]]))
        ...
        M_ = torch.cat(
            [
                torch.cat([M_[:, :2], torch.zeros(3, 1), M_[:, 2][..., None]], dim=-1),
                torch.zeros(1, 4),
            ]
        )
        M_[2, 2] = 1
        ...
        M_ = torch.cat(
            [
                torch.cat(
                    [M_inv[:, :2], torch.zeros(3, 1), M_inv[:, 2][..., None]], dim=-1
                ),
                torch.zeros(1, 4),
            ]
        )
        ...
        visible_mask = get_visible(
            img, gt2Dcrop[:, :2], cropSize=self.cropSize[0], background_value=1
        )  # joints that are visible within the image frame
    ...
    def cropArea3D(
        self, imgDepth, com, minRatioInside=0.75, size=(250, 250, 250), dsize=(128, 128)
    ):
        """
        ...
        xstart, xend, ystart, yend, zstart, zend = comToBounds(
            com.copy(), size, self.fx, self.fy
        )
        ...
        ratioInside = float((xendin - xstartin) * (yendin - ystartin)) / float(
        ...
        if (ratioInside < minRatioInside) and (
            ...
            or (com[1] < 0)
            or (com[1] >= imgDepth.shape[0])
        ):
            print(
                "Hand largely outside image (ratio (inside) = {})".format(ratioInside)
            )
            raise UserWarning("Hand not inside image")

        # crop patch from source
        cropped = imgDepth[
            max(ystart, 0) : min(yend, imgDepth.shape[0]),
            max(xstart, 0) : min(xend, imgDepth.shape[1]),
        ].copy()
        ...
        cropped = np.pad(
            ...
            (
                (
                    abs(ystart) - max(ystart, 0),
                    abs(yend) - min(yend, imgDepth.shape[0]),
                ),
                (
                    abs(xstart) - max(xstart, 0),
                    abs(xend) - min(xend, imgDepth.shape[1]),
                ),
            ),
            mode="constant",
            constant_values=int(CROP_BG_VALUE),
        )
...
class NYUHandPoseDataset(HandPoseDataset):
    def __init__(
        ...
        super(NYUHandPoseDataset, self).__init__(
            ...
            doAddWhiteNoise=doAddWhiteNoise,
        ...
        if center_refined:
            ...
            center_path = os.path.join(
                basepath, "center_{}_refined.txt".format(self.seqName)
            )
            refined = np.loadtxt(center_path)
            self.center_refined_uvd = np.array(
                refined
            )  # self.convert_xyz_to_uvd_tensor(torch.tensor(refined).unsqueeze(1))  ).squeeze() #(B,1, 3) -> (B,3)
    ...
    def LoadSample(self, ind, com=None):
        ...
        dptFileName = "{0:s}/{1:s}_{2:1d}_{3:07d}.png".format(
            objdir, prefix, self.camID, line + 1
        )
        ...
        self.randomComJitter = np.clip(
            np.random.randn(3) * 6, -self.comJitter, self.comJitter
        )  # (1 if self.comJitter else 0) * np.clip(np.random.randn(3)*6,-self.comJitter,self.comJitter)
        comGT = (
            copy.deepcopy(gt2Dorignal[idComGT]) if com is None else com
        ) + self.randomComJitter  # np.clip(np.concatenate((np.random.randn(2),np.array([0.])))*6,-6,6)

        if self.doJitterRotation:
            rotation_angle_scale = np.random.randn()
            rot = (
                rotation_angle_scale
                * (self.rotationAngleRange[1] - self.rotationAngleRange[0])
                + self.rotationAngleRange[0]
            )
            self.randomAngle = rot
            dpt, gt2Dorignal = rotateImageAndGt(
                dpt, comGT, rot, gt2Dorignal, bgValue=10000
            )

        # Jitter scale (cube size)?
        cubesize = np.float32(
            np.array(self.cropSize3D)
            * (5 / 6.0 if (not self.doTrain and ind >= 2440) else 1)
        )

        dpt, M, com = self.cropArea3D(
            imgDepth=dpt,
            com=comGT,
            minRatioInside=0.6,
            size=cubesize,
...
InvalidIndicies = [6635, 8001, 9990, 10292, 12378, 19770, 19863, 20531] + [
    7037,
    8724,
    11852,
    19161,
    ...
    20343,
    20532,
]  # the second for the case where we have center_jitter
...
class ICVLHandPoseDataset(HandPoseDataset):
    def __init__(
        ...
        self.fx, self.fy, self.ux, self.uy = (
            240.99,
            240.96,
            160,
            120,
        )  # (241.42, 241.42, 160., 120.)
        ...
        super(ICVLHandPoseDataset, self).__init__(
            basepath=basepath,
            train=train,
            cropSize=cropSize,
            doJitterRotation=doJitterRotation,
    ...
    def LoadSample(self, ind, com=None):
        ...
        self.randomComJitter = np.clip(
            np.random.randn(3) * 6, -self.comJitter, self.comJitter
        )  # (1 if self.comJitter else 0)* np.clip(np.random.randn(3)*6,-6,6)
        comGT = (
            copy.deepcopy(gt2Dorignal[idComGT]) if com is None else com
        ) + self.randomComJitter  # np.clip(np.concatenate((np.random.randn(2),np.array([0.])))*6,-6,6)
...
class MSRAHandPoseDataset(HandPoseDataset):
    def __init__(
        ...
        self.fx, self.fy, self.ux, self.uy = (241.42, 241.42, 160.0, 120.0)
        ...
        gt = open(self.address_gtUVD)
        gt.seek(0)
        gt_lines = gt.readlines()

        files = open(self.address_files)
        files.seek(0)
        ...
        for i in range(len(file_lines)):
            ...
            data[subject] = data.get(subject, []) + [
                (gt_uvd_original, subject, depth_address)
            ]
        ...
        super(MSRAHandPoseDataset, self).__init__(
            basepath=basepath,
            train=train,
            cropSize=cropSize,
            doJitterRotation=doJitterRotation,
    ...
    def LoadSample(self, ind, com=None):
        ...
        # Jitter scale (cube size)?
        cubesize = (
            self.default_cubes[subject] if self.use_default_cube else self.cropSize3D
        )
...
def comToBounds(com, size, fx, fy):
    ...
    xstart = int(np.floor((com[0] * com[2] / fx - size[0] / 2.0) / com[2] * fx + 0.5))
    xend = int(np.floor((com[0] * com[2] / fx + size[0] / 2.0) / com[2] * fx + 0.5))
    ystart = int(np.floor((com[1] * com[2] / fy - size[1] / 2.0) / com[2] * fy + 0.5))
    yend = int(np.floor((com[1] * com[2] / fy + size[1] / 2.0) / com[2] * fy + 0.5))
    return xstart, xend, ystart, yend, zstart, zend
...
def get_visible(img, landmarks, cropSize=128, background_value=1, win_size=4):
    ...
    for j in range(landmarks.shape[0]):
        ...
        if x >= 0 and x < cropSize and y >= 0 and y < cropSize:
            left = max(0, x - win_size)
            right = min(x + win_size, cropSize)
            bottom = max(0, y - win_size)
            top = min(y + win_size, cropSize)
...
def normalizeZeroOne(sample):
    ...
    imgD[imgD == 0] = sample.com[2] + (sample["cubesize"][2] / 2.0)
    imgD -= sample["com3D"][2] - (sample["cubesize"][2] / 2.0)
    imgD /= sample["cubesize"][2]

    target = (
        np.clip(
            np.asarray(sample["gt3Dcrop"], dtype="float32") / sample["cubesize"][2],
            -0.5,
            0.5,
        )
        + 0.5
...
def normalizeMinusOneOne(sample):
    imgD = np.asarray(sample["dpt"].copy(), "float32")
    imgD[imgD == 0] = sample["com3D"][2] + (sample["cubesize"][2] / 2.0)
    imgD -= sample["com3D"][2]
    imgD /= sample["cubesize"][2] / 2.0

    target = np.clip(
        np.asarray(sample["gt3Dcrop"], dtype="float32") / (sample["cubesize"][2] / 2.0),
        -1,
        1,
    )
    return imgD, target
...
def CropToOriginal(preds, matrices):
    ...
    v = torch.cat(
        [preds, torch.ones(preds.shape[0], preds.shape[1], 1).to(preds.device)], dim=-1
    )
...
def scale_depth(img, uvd, scale, background_value=1):
    ...
    dst = cv2.warpAffine(
        img, M, (cols, rows), borderMode=cv2.BORDER_CONSTANT, borderValue=1
    )
