...
class HandposeEvaluation(object):
    ...
    def getMeanError(self):
        ...
        """
        return np.nanmean(
            np.nanmean(np.sqrt(np.square(self.gt - self.joints).sum(axis=2)), axis=1)
        )
    ...
    def getNumFramesWithinMaxDist(self, dist):
        ...
        """
        return (
            np.nanmax(np.sqrt(np.square(self.gt - self.joints).sum(axis=2)), axis=1)
            <= dist
        ).sum()
    ...
    def getErrorPerDimension(self, printOut=True):
        ...
        msg = (
            "mean along X direction = %.2f \nmean along Y direction = %.2f\nmean along Z direction = %.2f"
            % (xmean, ymean, zmean)
        )
        if printOut:
            print(msg)
...
def draw_pose(img, pose, dataset="nyu"):
    ...
    for pt in pose:
        cv2.circle(
            img, (int(pt[0]), int(pt[1])), 2, colors_joint[idx].value, thickness=-1
        )
        idx = idx + 1
    idx = 0
